...
#define SOLUTION_START_ADDRESS ((void*)0x0000000020000000)
...
static void __attribute__((destructor)) count_points(void) 
{
    ...
    int points_received = maximum_points - tests_failed - tests_missing;
...
{
    assert(offset_counter < MAX_OFFSETS && "address wont fint into array");
...
static inline void* execute_malloc(size_t size, int init_value, int index, void* expected_return)
{
    ...
    allocated_memory = (unsigned char*)malloc(size);
...
void meta_data_test(void)
{
    ...
    for(memory_block_t* free_space = root->free_space_list_entry_point; 
        free_space != NULL; 
